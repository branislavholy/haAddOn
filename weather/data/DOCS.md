...
podman build -f debug/Dockerfile --format docker --pull --rm --build-arg BUILD_FROM="ghcr.io/home-assistant/aarch64-base:latest" -t weather:latest -t 'weather:1.8.15' .
...
```bash
cd ~/workspace/haAddOn/weather
podman build --format docker --pull --rm --build-arg BUILD_FROM="ghcr.io/home-assistant/aarch64-base:latest" -t weather:latest -t 'weather:1.8.15' .
...
<!-- x-release-please-start-version -->
```bash
podman rmi weather:1.8.15 weather:latest
// remove all images
